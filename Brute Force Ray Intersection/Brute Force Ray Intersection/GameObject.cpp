...
GameObject::GameObject()
{
	...
	//calls the function to load the object and store values 
	//within the variables of the GameObject class
...
//function used to load an object and store the vector positions 
//and highest and lowest coordinate values
	...
	if (inputFile.is_open())
	{
		...
		while (std::getline(inputFile, currentLine))
		{
			...
			if (!currentLine.substr(0, 2).compare(0, 1, "v"))
			{
				...
				//if statements used to create a bounding box for the object
				if (curx > m_highestx)
				{
					m_highestx = curx;
				}
